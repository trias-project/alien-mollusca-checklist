...
- `original_name_usage_id`: taxon_id of the _original_ scientific name if applicable
- `scientific_name`
- `accepted_name_usage`: the _accepted_ scientific name
- `original_name_usage`: the _original_ scientific name if applicable
...
Save the link between `scientific_name`, `accepted_name_usage`, `taxon_id` and `remarks` for original names (i.e. filter on `remarks` = `original name`) for future join:
...
Rearrange columns and add `taxonomy` (which is empty for `synonyms`) to enable future join with `accepted_taxa`:
...
Add original scientific names to `taxon`:
...
```{r}
write_csv(description, here("data","processed","description.csv"))
```
